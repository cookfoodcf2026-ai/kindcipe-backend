...
/** Parse a JSON-encoded tags column into an array of strings (never throws). */
...
/**
 * Aggregate all tags used across official recipes and the family's custom recipes.
 * Returns entries sorted by usage count (descending), then by name.
 */
export async function getAllRecipeTags(familyId: number) {
  const db = await getDb();
  if (!db) return [];
  const counts = new Map<string, number>();

  const official = await db.select({ tags: officialRecipes.tags }).from(officialRecipes)
  ...
  const custom = await db.select({ tags: customRecipes.tags }).from(customRecipes)
  ...
  return Array.from(counts.entries())
    ...
    .sort((a, b) => (b.count - a.count) || a.tag.localeCompare(b.tag, "zh-HK"));
}

/**
 * Remove a tag from every custom recipe in the family. Official recipes are
 * system-managed and are never modified here. Returns affected recipe count.
 */
export async function removeTagFromCustomRecipes(familyId: number, tag: string) {
  const db = await getDb();
  if (!db) throw new Error("DB unavailable");

  const recipes = await db.select({ id: customRecipes.id, tags: customRecipes.tags })
    .from(customRecipes).where(eq(customRecipes.familyId, familyId));
  ...
  for (const recipe of recipes) {
    ...
    await db.update(customRecipes).set({ tags: JSON.stringify(next) })
